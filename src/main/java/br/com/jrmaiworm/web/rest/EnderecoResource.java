...
import br.com.jrmaiworm.repository.EnderecoRepository;
...
/**
 * REST controller for managing {@link br.com.jrmaiworm.domain.Endereco}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class EnderecoResource {
    ...
    public List<Endereco> getAllEnderecos(@RequestParam(required = false) String filter) {
        if ("usuario-is-null".equals(filter)) {
            log.debug("REST request to get all Enderecos where usuario is null");
            return StreamSupport
                .stream(enderecoRepository.findAll().spliterator(), false)
                .filter(endereco -> endereco.getUsuario() == null)
